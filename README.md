...
- a signed-in private-account acceptance self-test that verifies the live RLS/CRUD/Storage/sync path without privileged credentials and cleans its synthetic probes automatically
...
- **Build 024 — private-account acceptance harness:** adds a self-cleaning signed-in infrastructure test for authenticated identity, anonymous-access rejection, normalized-event CRUD, preferences preservation, workout session/set continuity, photo metadata, private Storage upload/download/delete and the fully wrapped Sync Now pipeline. A small cloud acceptance marker is written only after every check passes.

Builds 019/021/022 implement the software continuity layer. Build 024 makes the next real-account acceptance step deterministic, but it does not replace the requirement to sign into the same real account in two browsers and prove actual Fuel/workout/photo reconstruction.
...
Build 024's acceptance harness uses only the same publishable key + authenticated user JWT available to the ordinary browser app. Its probes are uniquely tagged, cleaned in `finally`, and it restores the pre-existing preference row before the real Sync Now pass. It persists `settings.zero2fit_acceptance_v1` only after all infrastructure checks pass.
...
- `.github/workflows/validate-build024-acceptance.yml` — focused private-account acceptance helper/wiring/credential-boundary validation
...
2. run **Run acceptance self-test** and require every Build 024 check to pass;
3. sign into the same account on browser B and run the same self-test;
4. log a manual food and an Open Food Facts item on A, save a meal and set explicit nutrition targets;
5. Sync Now and reconstruct the Fuel data on B;
6. delete a food / clear a day and prove the tombstone prevents reappearance after bidirectional sync;
7. complete a workout on A and prove its set/load history follows the account and still drives the same adaptive recommendation on B;
8. capture/sync/delete a progress photo and prove private blob + metadata continuity in both directions.
