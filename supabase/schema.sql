-- Zero2Fit Build 002 target cloud schema.
-- This file prepares a future Supabase project; no project URL, keys or credentials belong in this repository.
...
create table if not exists public.user_preferences (
  ...
  workout_template text not null default 'foundation-a',
...
-- All application tables are private to auth.uid(). Policies are intentionally explicit.
do $$
declare
  table_name text;
begin
  foreach table_name in array array[
    'profiles','user_preferences','device_connections','normalized_events','import_runs',
    'workout_sessions','workout_sets','fitness_xp_ledger','rpg_state','progress_photo_sessions','progress_photo_assets'
  ]
  loop
...
-- Private progress-photo storage. Object names must start with the authenticated user UUID.
...
create policy "own progress photos update" on storage.objects
for update using (
  bucket_id = 'progress-photos'
  and (storage.foldername(name))[1] = auth.uid()::text
) with check (
...
-- Security rule: use the public anonymous key in a browser only with authenticated RLS.
