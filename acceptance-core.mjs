export const ACCEPTANCE_METRIC = 'acceptance_probe';
...
export function deviceEvidence(observations = [], verifications = [], manual = {}) {
  ...
  const verifiedMetrics = provider => {
    ...
    for (const row of providerRows(provider)) {
      const observed = bundles.get(row.source_bundle_id);
      for (const metric of observed || []) metrics.add(metric);
    }
    return metrics.size;
  };
  return {
...
export function acceptanceEventInput(snapshot, observedAt = new Date().toISOString()) {
  if (!snapshot?.browser_instance_id) throw new Error('Acceptance snapshot requires browser_instance_id.');
  return {
    metricType:ACCEPTANCE_METRIC,
    value:1,
    unit:'check',
    observedAt,
    sourceProvider:'zero2fit',
    sourceDevice:'web_app',
    sourceRecordId:`acceptance:${snapshot.browser_instance_id}`,
    provenanceStatus:'user-entered',
    confidence:'system_verified',
    metadata:{ acceptance_v1:snapshot }
  };
}

export function latestAcceptanceSnapshots(events = []) {
  const latest = new Map();
  for (const event of events || []) {
    if (event?.metric_type !== ACCEPTANCE_METRIC) continue;
    const snapshot = event?.metadata?.acceptance_v1;
...
export function crossBrowserEvidence(snapshots = []) {
  const clean = (snapshots || []).filter(row => row?.browser_instance_id);
...
export function build020Steps({ account = {}, fuel = {}, workout = {}, photos = {}, cross = {}, manual = {} } = {}) {
  return [
    ...
    { id:'sync', label:'Private sync completed', done:Boolean(fuel.synced) },
