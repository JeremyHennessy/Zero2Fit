(() => {
  ...
  function normalizeHeader(value) {
    return String(value || '').toLowerCase().replace(/%/g, 'percentage').replace(/[^a-z0-9]+/g, '');
  }

  function headerMatchesMetric(header, key) {
    if (header === key) return true;
    return ['lb','lbs','kg','percentage','pct','percent','level','kcal','years','bpm'].some(suffix => header === `${key}${suffix}`);
  }
  ...
  function makeEvent({ metricType, value, unit, observedAt, endAt = null, sourceProvider, sourceDevice = null, sourceRecordId = null, provenanceStatus = 'imported', confidence = 'imported', metadata = {} }) {
    const identity = sourceRecordId || `${sourceProvider}|${metricType}|${observedAt}|${endAt || ''}|${value}|${unit || ''}`;
    return {
  ...
  function parseRenphoCsv(text, fileName = 'renpho.csv') {
    ...
    rows.slice(1).forEach((row, rowIndex) => {
      ...
      metricColumns.forEach(({ definition, index, header }) => {
        ...
        const sourceRecordId = `renpho:${stableHash(`${observedAt}|${definition.metric}|${value}|${unit}`)}`;
        events.push(makeEvent({
          ...
          metadata:{ original_header:header, import_format:'renpho_csv', source_file:fileName, source_row:rowIndex + 2 }
        }));
      });
    });
  ...
  function normalizeAppleValue(type, value) {
    ...
    if (type === 'HKQuantityTypeIdentifierBodyFatPercentage' || type === 'HKQuantityTypeIdentifierOxygenSaturation') return numeric > 0 && numeric <= 1 ? numeric * 100 : numeric;
    return numeric;
  }

  function appleRecordIdentity(record, type) {
    return [
      type,
      record.getAttribute('startDate'), record.getAttribute('endDate'), record.getAttribute('value'),
      record.getAttribute('unit'), record.getAttribute('sourceName'), record.getAttribute('sourceVersion'), record.getAttribute('device')
    ].join('|');
  }

  function parseAppleHealthXml(text) {
    if (typeof DOMParser === 'undefined') return { events:[], warnings:['XML parsing is not available in this environment.'] };
    ...
    doc.querySelectorAll('Record').forEach(record => {
      ...
      if (!observedAt) return;
      const sourceId = `apple-record:${stableHash(appleRecordIdentity(record, type))}`;
      const mapping = appleTypeMap[type];
      if (!mapping) {
        if (type === 'HKCategoryTypeIdentifierSleepAnalysis') {
          events.push(makeEvent({
            metricType:'sleep_stage', value:record.getAttribute('value') || 'unknown', unit:'category', observedAt,
            endAt:normalizeDate(record.getAttribute('endDate')), sourceProvider:'apple_health', sourceDevice:record.getAttribute('device') || null,
            sourceRecordId:sourceId, confidence:'measured', metadata:{ source_name:record.getAttribute('sourceName'), apple_type:type }
          }));
        }
        return;
      }
      ...
      events.push(makeEvent({
        metricType, value:normalizeAppleValue(type, record.getAttribute('value')), unit:preferredUnit || originalUnit || 'unknown', observedAt,
        endAt:normalizeDate(record.getAttribute('endDate')), sourceProvider:'apple_health', sourceDevice:record.getAttribute('device') || null,
        sourceRecordId:sourceId, confidence,
        metadata:{ source_name:record.getAttribute('sourceName'), source_version:record.getAttribute('sourceVersion'), apple_type:type, original_unit:originalUnit }
      }));
    });
    ...
    doc.querySelectorAll('Workout').forEach(workout => {
      ...
      if (!observedAt || duration === null) return;
      const identity = [workout.getAttribute('workoutActivityType'), workout.getAttribute('startDate'), workout.getAttribute('endDate'), duration, workout.getAttribute('sourceName')].join('|');
      events.push(makeEvent({
        metricType:'workout_session', value:duration, unit:workout.getAttribute('durationUnit') || 'min', observedAt,
        endAt:normalizeDate(workout.getAttribute('endDate')), sourceProvider:'apple_health', sourceRecordId:`apple-workout:${stableHash(identity)}`, confidence:'measured',
        metadata:{
          activity_type:workout.getAttribute('workoutActivityType'), source_name:workout.getAttribute('sourceName'),
          total_energy_burned:parseNumber(workout.getAttribute('totalEnergyBurned')), total_energy_unit:workout.getAttribute('totalEnergyBurnedUnit')
        }
      }));
    });
  ...
  function normalizeBundle(json) {
    ...
    const events = sourceEvents.filter(event => event && event.metric_type && event.observed_at).map(event => {
      ...
      return makeEvent({
        metricType:event.metric_type, value:event.value, unit:event.unit, observedAt:normalizeDate(event.observed_at) || event.observed_at,
        endAt:event.end_at ? (normalizeDate(event.end_at) || event.end_at) : null, sourceProvider:event.source_provider || 'healthkit_bridge',
        sourceDevice:event.source_device || null, sourceRecordId:event.source_record_id || null,
        provenanceStatus:event.provenance_status || 'imported', confidence:event.confidence || 'imported', metadata:event.metadata || {}
      });
    });
    return { events, warnings:events.length ? [] : ['JSON file did not contain normalized Zero2Fit events.'] };
  }
  ...
  async function importFile(file, sourceHint = 'auto') {
    ...
    let parsed;
    let sourceProvider;
    if (sourceHint === 'renpho' || lowerName.endsWith('.csv')) { parsed = parseRenphoCsv(text, name); sourceProvider = 'renpho'; }
    else if (sourceHint === 'apple_health' || lowerName.endsWith('.xml')) { parsed = parseAppleHealthXml(text); sourceProvider = 'apple_health'; }
    ...
    } else throw new Error('Unsupported import type. Use RENPHO CSV, Apple Health export.xml, or normalized JSON.');

    const events = [...new Map(parsed.events.map(event => [event.event_id, event])).values()];
    ...
    return {
      ...
      importRecord:{
        ...
        imported_at:importedAt, source_provider:sourceProvider, file_name:name, file_size:file.size,
        event_count:events.length, warnings:parsed.warnings || []
      }
    };
  }

  window.Zero2FitIngestion = { makeEvent, parseRenphoCsv, parseAppleHealthXml, normalizeBundle, importFile };
})();
