...
function ensureSettings() {
  ...
  button.id = 'z12SettingsButton';
  button.className = 'icon-button z12-settings-button';
  button.type = 'button';
  button.title = 'Settings and devices';
  button.setAttribute('aria-label','Settings and devices');
  button.textContent = '⚙';
  actions.appendChild(button);

  const backdrop = document.createElement('div');
  backdrop.id = 'z12SettingsBackdrop';
  backdrop.className = 'z12-settings-backdrop';
  backdrop.hidden = true;
  backdrop.innerHTML = `
    <section class="z12-settings-sheet" role="dialog" aria-modal="true" aria-labelledby="z12SettingsTitle">
      <div class="z12-settings-head">
        <div><div class="eyebrow">Zero2Fit</div><h2 id="z12SettingsTitle">Settings</h2></div>
        <button class="icon-button" type="button" id="z12SettingsClose" aria-label="Close settings">×</button>
      </div>
  ...
  if (reset) {
    reset.classList.add('z12-reset-action');
    reset.textContent = 'Reset local Zero2Fit data';
    reset.title = 'Reset local Zero2Fit data';
    backdrop.querySelector('.z12-settings-sheet')?.appendChild(reset);
  }
  ...
  backdrop.querySelectorAll('[data-z12-page]').forEach(link => link.addEventListener('click', () => {
    closeSettings();
    document.querySelector(`.nav-item[data-page="${link.dataset.z12Page}"]`)?.click();
  }));
...
function updateOfflineStatus() {
  const node = document.getElementById('z12OfflineStatus');
  if (!node) return;
...
function markStrengthBlocks() {
  ...
  const records = document.getElementById('z10Records');
  if (!intel || !records) return;
  records.classList.add('z12-strength-block');
  const heading = [...intel.children].find(node => node.classList?.contains('eyebrow') && /personal records/i.test(node.textContent || ''));
  heading?.classList.add('z12-strength-block');
}
...
function ensureProgressTabs() {
  ...
  tabs.id = 'z12ProgressTabs';
  tabs.className = 'z12-progress-tabs';
  tabs.setAttribute('aria-label','Progress sections');
  tabs.innerHTML = `
    <button type="button" data-z12-progress="overview">Overview</button>
    <button type="button" data-z12-progress="strength">Strength</button>
    <button type="button" data-z12-progress="body">Body</button>
    <button type="button" data-z12-progress="photos">Photos</button>`;
  hero.after(tabs);
  tabs.querySelectorAll('[data-z12-progress]').forEach(button => button.addEventListener('click', () => {
    progressTab = button.dataset.z12Progress;
    sessionStorage.setItem('zero2fit-progress-tab', progressTab);
    applyProgressTab();
  }));
  markStrengthBlocks();
  applyProgressTab();
...
function setGridChildren(grid, predicate) {
  if (!grid) return;
  [...grid.children].forEach((child, index) => { child.hidden = !predicate(child, index); });
}
...
function applyProgressTab() {
  ...
  const photos = document.getElementById('z4PhotoTrackerPreview');
  const grid = document.querySelector('#page-journey > .content-grid');
  markStrengthBlocks();

  if (!mobile()) {
    [intel, body, photos, grid].forEach(node => { if (node) node.hidden = false; });
    intel?.classList.remove('z12-strength-view');
    setGridChildren(grid, () => true);
    return;
  }

  if (intel) {
    intel.hidden = !['overview','strength'].includes(progressTab);
    intel.classList.toggle('z12-strength-view', progressTab === 'strength');
  }
  if (body) body.hidden = progressTab !== 'body';
  if (photos) photos.hidden = progressTab !== 'photos';
  if (grid) {
    grid.hidden = progressTab === 'strength' || progressTab === 'photos';
    if (progressTab === 'overview') setGridChildren(grid, () => true);
    else if (progressTab === 'body') setGridChildren(grid, child => /weight history/i.test(child.querySelector('h2')?.textContent || ''));
  }
}
...
function adventureTargets() {
  return {
    stats: document.querySelector('#z11AdventureStatus > section:first-child'),
    auto: document.querySelector('#page-character .z7-auto-panel'),
    materials: document.querySelector('#z11AdventureStatus > section:nth-child(2)'),
    log: [...document.querySelectorAll('#page-character .z7-lower-grid')].find(node => node.id !== 'z11AdventureStatus') || null
  };
}

function applyAdventurePanel() {
  const controls = document.getElementById('z12AdventureControls');
  if (!controls) return;
  ...
  const targets = adventureTargets();
