import assert from 'node:assert/strict';
import {
  ACCEPTANCE_METRIC,
  fuelEvidence,
  workoutEvidence,
  photoEvidence,
  deviceEvidence,
  acceptanceEventInput,
  latestAcceptanceSnapshots,
  crossBrowserEvidence,
  build020Steps,
  physicalDeviceSteps,
  summarizeSteps
} from '../acceptance-core.mjs';

const fuel = {
  meals:{
    '2026-09-01':[
      { id:'m1', source:'quick_line', calories:500 },
      { id:'m2', source:'open_food_facts', calories:200 }
    ]
  },
  savedMeals:[{id:'saved'}],
  nutritionTargets:{calories:2200,protein:180,carbs:200,fat:70}
};
const fuelEvents = [
  { metric_type:'nutrition_entry', source_provider:'open_food_facts' },
  { metric_type:'nutrition_entry_deleted' }
];
const fuelResult = fuelEvidence(fuel, fuelEvents, { synced_at:'2026-09-01T01:00:00Z', fuel_history_entries:2, fuel_deleted_entries:1 });
assert.equal(fuelResult.manual_entries, 1);
assert.equal(fuelResult.provider_entries, 1);
assert.equal(fuelResult.saved_meals, 1);
assert.equal(fuelResult.targets_set, 4);
assert.equal(fuelResult.tombstones, 1);
assert.equal(fuelResult.synced, true);

const workoutState = {
  completedWorkouts:2,
  workoutHistory:[{day:'2026-08-30'},{day:'2026-09-01'}],
  exerciseHistory:[
    { day:'2026-09-01', exerciseId:'dumbbell-row', completedSets:3, workingLoad:35, maxReps:12, sets:[{reps:12,load:35,done:true}] },
    { day:'2026-08-30', exerciseId:'dumbbell-row', completedSets:3, workingLoad:35, maxReps:12, sets:[{reps:12,load:35,done:true}] }
  ]
};
const workoutResult = workoutEvidence(workoutState, { synced_at:'2026-09-01T01:00:00Z', workout_sessions:2, workout_sets:6 });
assert.equal(workoutResult.synced, true);
assert.equal(workoutResult.synced_sets, 6);
assert.ok(workoutResult.history_signature?.includes('dumbbell-row'));

const photoResult = photoEvidence([{photo_id:'p1'}], [{metric_type:'progress_photo_deleted'}], {
  progress_photo_remote_assets:1,
  progress_photo_uploaded:1,
  progress_photo_downloaded:0,
  progress_photo_deleted_assets:1
});
assert.equal(photoResult.local_assets, 1);
assert.equal(photoResult.tombstones, 1);
assert.equal(photoResult.ever_uploaded, true);
assert.equal(photoResult.ever_deleted_remote, true);

const devices = deviceEvidence(
  [
    { source_bundle_id:'zepp.bundle', metric_type:'steps' },
    { source_bundle_id:'renpho.bundle', metric_type:'weight' }
  ],
  [
    { provider:'zepp', source_bundle_id:'zepp.bundle' },
    { provider:'renpho', source_bundle_id:'renpho.bundle' }
  ],
  { healthkit_value_parity:true, healthkit_background_delivery:true, renpho_model_label:true }
);
assert.equal(devices.zepp_verified, true);
assert.equal(devices.renpho_verified, true);
assert.equal(devices.zepp_metric_types, 1);
assert.equal(physicalDeviceSteps(devices).every(step => step.done), true);

const snapshotA = {
  browser_instance_id:'browser-a',
  fuel:{entries:2,synced:true,tombstones:1},
  workout:{history_rows:2,synced:true,history_signature:'same-history'},
  photos:{ever_uploaded:true,ever_downloaded:false,ever_deleted_remote:true,tombstones:1}
};
const snapshotB = {
  browser_instance_id:'browser-b',
  fuel:{entries:2,synced:true,tombstones:1},
  workout:{history_rows:2,synced:true,history_signature:'same-history'},
  photos:{ever_uploaded:false,ever_downloaded:true,ever_deleted_remote:false,tombstones:1}
};
const cross = crossBrowserEvidence([snapshotA,snapshotB]);
assert.deepEqual(cross, {
  browser_count:2,
  fuel_reconstructed:true,
  fuel_deletion_propagated:true,
  workout_reconstructed:true,
  matching_workout_history:true,
  photo_round_trip:true,
  photo_deletion_propagated:true
});

const steps = build020Steps({
  account:{signed_in:true},
  fuel:fuelResult,
  workout:workoutResult,
  photos:photoResult,
  cross,
  manual:{adaptive_second_browser_confirmed:true}
});
assert.equal(steps.length, 10);
assert.equal(steps.every(step => step.done), true);
assert.deepEqual(summarizeSteps(steps), {complete:10,partial:0,total:10,done:true});

const eventInput = acceptanceEventInput(snapshotA, '2026-09-01T01:02:03Z');
assert.equal(eventInput.metricType, ACCEPTANCE_METRIC);
assert.equal(eventInput.sourceRecordId, 'acceptance:browser-a');
assert.equal(eventInput.metadata.acceptance_v1.browser_instance_id, 'browser-a');

const latest = latestAcceptanceSnapshots([
  { metric_type:ACCEPTANCE_METRIC, observed_at:'2026-09-01T01:00:00Z', metadata:{acceptance_v1:{browser_instance_id:'browser-a',version:1}} },
  { metric_type:ACCEPTANCE_METRIC, observed_at:'2026-09-01T02:00:00Z', metadata:{acceptance_v1:{browser_instance_id:'browser-a',version:2}} },
  { metric_type:ACCEPTANCE_METRIC, observed_at:'2026-09-01T01:30:00Z', metadata:{acceptance_v1:{browser_instance_id:'browser-b',version:1}} }
]);
assert.equal(latest.length, 2);
assert.equal(latest.find(row => row.browser_instance_id === 'browser-a').version, 2);

console.log('Build 024 acceptance-core tests passed.');
