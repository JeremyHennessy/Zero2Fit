...
capture_dom() {
  ...
  for attempt in 1 2; do
    ...
    if [[ "$chrome_status" -eq 0 ]] && [[ -s "$DOM_FILE" ]] && grep -Fq 'id="z11AdventureStatus"' "$DOM_FILE" && grep -Fq 'id="z28HealthKitEvidence"' "$DOM_FILE" && grep -Fq 'id="z43TuningSignals"' "$DOM_FILE" && grep -Fq 'data-zero2fit-training-friction="ready"' "$DOM_FILE" && grep -Fq 'data-zero2fit-fuel-friction="ready"' "$DOM_FILE" && grep -Fq 'data-zero2fit-tuning-review="ready"' "$DOM_FILE"; then
...
assert_dom 'data-zero2fit-fuel-friction="ready"' 'Build 045 Fuel-friction readiness'
assert_dom 'id="z45ToggleMeasurement"' 'Build 045 measurement control'
assert_dom './build045.css' 'Build 045 stylesheet'
assert_dom 'data-zero2fit-tuning-review="ready"' 'Build 046 tuning-review readiness'
assert_dom 'id="z46SignalReview"' 'Build 046 tuning review shell'
assert_dom 'id="z46SignalToggle"' 'Build 046 tuning review toggle'
assert_dom './build046.css' 'Build 046 stylesheet'
