...
export function stagePathModel(status = {}) {
  ...
  const nodes = Array.from({ length:stageCount }, (_, index) => {
    ...
    return {
      type:'stage', stage:stageNumber, label:`Stage ${stageNumber}`,
      completed, current:!cleared && stageNumber === currentStage,
      locked:!completed && stageNumber > currentStage, threshold
    };
  });
  nodes.push({
    type:'boss', label:'Boss', completed:cleared,
...
function expectedEnemy(status, catalog) {
  ...
  const candidates = (zone.enemyIds || [])
    .map(id => enemyById(catalog, id)).filter(Boolean)
...
export function encounterVisualModel({ status = {}, adventure = {}, catalog = {} } = {}) {
  ...
  if (lastBattle) {
    ...
    return {
      source:'last_battle', id:definition.id,
      ...
      enemyHp:num(lastBattle.enemyHp), enemyMaxHp:maxHp,
      enemyHpPercent:clamp(Math.round(num(lastBattle.enemyHp) / maxHp * 100), 0, 100),
      playerHp:num(lastBattle.playerHp), playerMaxHp,
      playerHpPercent:clamp(Math.round(num(lastBattle.playerHp) / playerMaxHp * 100), 0, 100),
      turns:num(lastBattle.turns), ...visual
    };
  }
  const definition = expectedEnemy(status, catalog);
  if (!definition) return null;
  return {
    source:'encounter_pool', id:definition.id, name:definition.name,
    boss:Boolean(definition.boss), stage:num(definition.stage || status?.stage?.stage),
    outcome:status?.stage?.bossDue ? 'Boss ahead' : 'Threat ahead',
    enemyHp:num(definition.hp), enemyMaxHp:num(definition.hp), enemyHpPercent:100,
    playerHp:num(status?.capabilities?.maxHp), playerMaxHp:num(status?.capabilities?.maxHp), playerHpPercent:100,
    turns:0, ...enemyVisualKind(definition)
  };
}

export function gearVisualModel(adventure = {}, status = {}) {
  ...
  return ['weapon','armor','charm'].map(slot => {
    ...
    return {
      slot, name:item?.name || 'Empty',
      icon:item?.icon || (slot === 'weapon' ? '⚔' : slot === 'armor' ? '⬟' : '◆'),
      rarity:item?.rarity || 'none', rawPower, effectivePower,
      lockedPower:Math.max(0, rawPower - effectivePower), equipped:Boolean(item)
    };
  });
}
...
export function oddsLevers(status = {}) {
  ...
  if (num(caps.lockedGearPower) > 0) {
    rows.push({ key:'gear_ceiling', title:'Unlock equipped gear power', detail:`${num(caps.lockedGearPower)} equipped gear power is banked behind real-world capability ceilings.`, priority:100 });
  }
  if (wall?.type === 'capability_gate') {
    for (const requirement of wall.requirements || []) {
      rows.push({
        ...
        detail:requirement.type === 'fitness_level' ? `Current ${requirement.current}; next zone requires ${requirement.required}.` : `Current ${requirement.current} wins; ${requirement.required} are required.`,
        priority:110
      });
    }
  }
...
export function rewardRevealModel(adventure = {}, catalog = {}) {
  ...
  return {
    hasResult:true, coins:num(result.coinsEarned),
    loot:(result.loot || []).map(item => ({ name:item.name, icon:item.icon || '◆', rarity:item.rarity || 'common', power:num(item.power), slot:item.slot })),
    materials:Object.entries(result.materialsEarned || {}).filter(([, qty]) => num(qty) > 0).map(([id, qty]) => ({
      id, quantity:num(qty), name:materialDefs.get(id)?.name || id.replaceAll('_',' '), icon:materialDefs.get(id)?.icon || '◆'
    }))
  };
}
